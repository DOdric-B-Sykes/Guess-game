...
const playAgainButton = document.querySelector(".play-again"); /* The document.querySelector() element targets the first element that matches the selector (meaning since the play again button class element is coded as ".play-again hide", the selector targets the "play-again" part without the "hide" since the play again comes before hide) */
...
let letterGuesses = []; /* Variable to represent the letters guessed by the player in the form of an array; different than the 'guessedLetters" variable */
...
const getWord = async function(){
    ...
    const newWordArray = data.split("\n"); /* data fetched from the .text file (wordRequest variable) split into newlines making an array with each string on its own line within the array */
    console.log(newWordArray);

    const randomWordIndex = Math.floor(Math.random() * newWordArray.length); /* Used to pull random index from "newWordArray" */
    const randomWord = newWordArray[randomWordIndex]; /* Used to pull random string (word) from the "newWordArray" based on the random index generated by "randomWordIndex" */
    console.log(randomWord);

    const newWord = randomWord.trim(); /* Used to remove whitespace around random word pulled by the "randomWord" variable */
    word = newWord;

    placeholders(newWord); /* Calls (runs) the placeholder function, using the value of the "newWord" variable as an argument (the random word selected via "newWrord" will be used in the placeholder function) */ 
};
...
const gameWon = function (wordUpper){ /* Using wordUpper as a parameter, I can use the value of "wordUpper" from my updateWord function for my gameWon function -> this helps define "wordUpper" when used in the conditional statement (wordUpper === guessInProgress.innerText) */
   if (wordUpper === guessInProgress.innerText){ 
    ...
    startOver(wordUpper); /* Calls (runs) the startOver function at the end of the gameWon function when the gameWon function is called */
};

/* Function to trigger the play again button once the game ends */
const startOver = function (wordUpper){
    if (wordUpper === guessInProgress.innerText){
        guessButton.classList.add("hide"); /* Adds the hide css property to element in front of the .classList property */
        remainingGuesses.classList.add("hide");
        guessedLetters.classList.add("hide");
        playAgainButton.classList.remove("hide"); /* Using classList.remove("hide") -> removes the hide css property from the the Play Again button (which is targeting the play again button element via document.querySelector(".play-again") -> since the play again button element class is ".play-again hide" in the HTML, the hide css is removed) */
        ...
        console.log("Please keep playing");
    }
}; 

playAgainButton.addEventListener("click", function(){
    messageForGuesses.classList.remove("win");
    messageForGuesses.innerHTML = "";
    guessedLetters.innerHTML = "";
    guessesLeft = 8;
    letterGuesses = [];
    spanRemainingGuesses.innerText = `You have ${guessesLeft} remaining`;
